...
/**
 * Finalize an application and then send to secretary office as approved or rejected
 * @property {number} applId
 * @example <FinalizeApplication applId={this.props.applId} />
 */
class FinalizeApplication extends Component{
    constructor(props){
        super(props)
        this.state={
            alertColor:"info",
    ...
    loadData(){
        ...
        Fetchers.postJSON("/api/moderator/application/open",this.state.data,(query,result)=>{
            if(result.navigator.component.toUpperCase() == "FINALIZEAPPLICATION"){
     ...
     */
    approval(){
        Alerts.warning(this.state.labels.approval, ()=>{
        this.state.data.fields=["approvalDate","expiry_date","approver"] //validate them
        Fetchers.postJSON("/api/moderator/application/approval", this.state.data, (query,result)=>{
            this.submitAction(result)
        })}, ()=>{
     ...
     */
    return(){
        Alerts.warning(this.state.labels.return, ()=>{
            this.state.data.fields=[]
        Fetchers.postJSON("/api/moderator/application/return", this.state.data, (query,result)=>{
           this.submitAction(result)
        })}, ()=>{
        ...
        return(
            <Card style={{fontSize:"0.8rem"}}>
                <CardHeader>
                    ...
                    <Col xs='12' sm='12' lg='2' xl='2'>
                           <ButtonUni onClick={this.approval} label={this.state.labels.approval} outline={!ValidationMarkup.markedAsValid(this.state.data)}
                            disabled={!ValidationMarkup.markedAsValid(this.state.data)} />
                    </Col>
                    <Col xs='12' sm='12' lg='2' xl='2'>
                           <ButtonUni onClick={this.return} label={this.state.labels.return} outline={false} />
                    </Col>
                    <Col xs='12' sm='12' lg='2' xl='2'>
                    ...
                    <Row>
                        ...
                        <Col xs='12' sm='12' lg='6' xl='6'>
                            ...
                            <Row hidden={!ValidationMarkup.markedAsValid(this.state.data)}>
                                <Col>
                                    <FinalizeMarkup appId={this.props.appId} readOnly={false} strict={!this.state.data.valid}/>
                                </Col>
                            </Row>
