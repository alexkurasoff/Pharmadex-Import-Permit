...
class TableHeader extends Component{
    ...
    createSortArrows(){
        ...
        switch(this.props.header.sortValue){
            case 0:
            ret.push(
                <h1  key="1" className="p-0 m-0 mr-2 fa fa-times"></h1>)
                ret.push(<Button key="2" color='link' onClick={this.descSort} className="p-0 m-0 mr-2 fa fa-arrow-down"></Button>)
                ret.push(<Button key="3" color='link' onClick={this.ascSort} className="p-0 m-0 mr-2 fa fa-arrow-up"></Button>)
                break;
            case 1:
                ret.push(<Button key="1" color='link' onClick={this.noSort} className="p-0 m-0 mr-2 fa fa-times"></Button>)
                ret.push(<h1 key="2" className="p-0 m-0 mr-2 fa fa-arrow-down"></h1>)
                ret.push(<Button key="3" color='link' onClick={this.ascSort} className="p-0 m-0 mr-2 fa fa-arrow-up"></Button>)
                break;
            case 2:
                ret.push(<Button key="1" color='link' onClick={this.noSort} className="p-0 m-0 mr-2 fa fa-times"></Button>)
                ret.push(<Button key="2" color='link' onClick={this.descSort} className="p-0 m-0 mr-2 fa fa-arrow-down"></Button>)
                ret.push(<h1 key="3" className="p-0 m-0 mr-2 fa fa-arrow-up"></h1>)
            default:
        }
    ...
    additionTokens(){
        ...
        if(this.props.header.sortValue==2){
            ret.push(<span key="2" style={{fontSize:'0.5rem'}} className="fa fa-arrow-up"/>)
        }
        if(this.props.header.sortValue==1){
            ret.push(<span key="1" style={{fontSize:'0.5rem'}} className="fa fa-arrow-down"/>)
        }
        if(this.props.header.filterActive){
            ret.push(<span key="3" style={{fontSize:'0.5rem'}} className="fa fa-filter"/>)
        }
    ...
    rightShiftButton(){
        ...
        if(typeof this.props.rightShift =='function' ){
            ret.push(
                ...
                style={{backgroundColor: this.headBackground,  color: '#fff', fontSize:'0.7rem'}}
                className={"p-0 m-0"}
        ...
        if(header.sort || header.filterAllowed){
            return(
                ...
                <div style={{display:'flex',flexWrap:'noWrap'}}>
                    ...
                    style={{backgroundColor: this.headBackground,  color: '#fff',  whiteSpace:'normal', fontSize:'0.7rem'}}
    ...
    paintHeader(){
        ...
        if(element.important){
            return(<th style={this.inlineStyle(element.key)} key={element.key} className={"font-weight-light text-center align-middle"}>{this.headerContent(element)}</th>);
        }else{
            return(<th style={this.inlineStyle(element.key)} className={"font-weight-light text-center align-middle " + this.props.notImportantClasses()} key={element.key}>{this.headerContent(element)}</th>);
        }
    }
    render(){
        return(
