...
class FieldOracle extends Component{
    ...
    onSearchChange(e){
        ...
        if(value.length>=3){  //TRESHOLD TRESHOLD TRESHOLD
    ...
    loadOptions(searchString){
        ...
        Fetchers.postJSONNoSpinner(this.props.api, askFor,(query,option)=>{
            ...
            s.page=1
            s.pages=Math.round(s.originalOptions.length / this.countOnPage)
            if(s.pages==0){
                s.pages=1
            }
    ...
    createOptions(){
        ...
        let start=(this.state.page-1) * this.countOnPage
        let end = start + this.countOnPage
    ...
    render(){
        ...
        if(!isUndefined(this.data) && !isUndefined(this.data[this.attribute]) && !isUndefined(this.data[this.attribute].value)){
            ...
            this.props.component.state.data[this.props.attribute].justloaded=false
            return(
                <FormGroup>
                    <Label for={key}>
